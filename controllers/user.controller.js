...
function uid() {
  return (
    new Date(Date.now()).getTime() + Math.floor(Math.random() * (99 + 10) + 10)
  );
}
...
export const login = async (req, res) => {
  ...
  const passwordInDb = CryptoJS.AES.decrypt(
    user.password,
  ...
  const tokenCreated = jwt.sign(
    { id: user.id, username: originalUsername, password: orginalPswrd },
    process.env.secretKey,
    { expiresIn: "24h" }
  );
...
export const followManager = async (req, res) => {
  ...
  if (user.following.includes(followTo.id))
    user.following = user.following.filter((id) => id !== followTo.id);
  else user.following.push(followTo.id);

  if (followTo.followers.includes(user.id))
    followTo.followers = followTo.followers.filter((id) => id !== user.id);
...
export const uploadImages = async (req, res) => {
  ...
  const user = await userSchema.findOne({
    id: req.files[0].originalname.split("-")[1],
  });
  if (!user) return res.status(500).send("err");
